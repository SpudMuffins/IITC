...
(function() {
  ...
  // Monitor changes to highlighter selection
  function watchHighlighterChanges() {
    const origSetHighlighter = window.setActiveHighlighter;
    window.setActiveHighlighter = function(name) {
      ...
      return origSetHighlighter.apply(this, arguments);
    };

    // Also show UI if it's already the active one at load time
    if (window._currentHighlighter === HIGHLIGHTER_NAME) {
      createLegendBox();
    }
  }

  // Register the highlighter
  window.addPortalHighlighter(HIGHLIGHTER_NAME, highlightPortal);
  watchHighlighterChanges();
